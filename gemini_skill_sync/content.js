...
(async () => {
    ...
    const data = await browser.storage.local.get("registry");
...
function renderSkills() {
    ...
    if (registry.skills.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#aaa; margin-top:20px;">No skills loaded.</div>';
        return;
    }
...
async function updateCategory(index, newCat) {
    ...
    await saveRegistry();
    renderSkills(); // Re-render to update grouping
}
...
async function handleImport(e) {
    ...
    for (const file of skillFiles) {
        ...
        // webkitRelativePath example: "MySkills/Coding/Python/SKILL.md"
        const parts = file.webkitRelativePath.split('/');
        // Use the folder name containing the SKILL.md file as the skill name
        const folderName = parts.length > 1 ? parts[parts.length - 2] : 'Unknown';
        const category = parts.length > 2 ? parts[0] : 'General'; // Very basic category guess
...
async function saveRegistry() {
    await browser.storage.local.set({ registry });
}

function injectText(text) {
    // Try to find the Gemini input box
    const inputField = document.querySelector('div[contenteditable="true"]') ||
        document.querySelector('textarea[aria-label="Prompt"]');

    if (inputField) {
        inputField.focus();
        // Simulate user typing/pasting
        // We wrap it to make it clear it's a skill instruction
